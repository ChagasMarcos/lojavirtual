...
public class AccountController(SignInManager<AppUser> signInManager) : BaseApiController
{
    ...
    public async Task<ActionResult> Register(RegisterDto registerDto)
    {
        ...
        if (!result.Succeeded)
        {
                foreach(var error in result.Errors)
                    {
                        string descricaoTraduzida = error.Code switch
                    {
                        "PasswordRequiresDigit" => "A senha deve conter pelo menos um dígito ('0'-'9').",
                        ...
                        _ => error.Description // Caso seja outro erro não mapeado, mantém o padrão
                    };

                    ModelState.AddModelError(error.Code, descricaoTraduzida);
                }
    ...
    public async Task<ActionResult<AddressDto>> CreateOrUpdateAddress(AddressDto addressDto)
    {
       ...
       if(user.Address == null)
        ...
        if(!result.Succeeded) return BadRequest("Não foi possível atualizar o endereço.");
